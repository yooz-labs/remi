...
/**
 * Identity adoption must not depend on option shape (#887 review finding).
 *
 * `consumeAndMerge` originally adopted the hook's id only inside the
 * `hookRecord.options.length > 0` branch. That is unreachable today because
 * every producer in `hook-event-bridge.ts` guarantees at least the honest
 * Yes/No pair -- but nothing enforces it, and the defensive net that used to
 * catch the mismatch (`AutoApproveGate.rekeySignatureToRendered`) is deleted.
 *
 * `openQuestionSignatures` is keyed by the hook's id at park time, so a merged
 * card carrying the PTY's id instead would silently reintroduce the #808
 * stale-card class. This pins that identity and option-shape stay independent.
 */
describe('id adoption is independent of option shape (#887)', () => {
  test('an optionless hook record still yields the hook id', () => {
    const tracker = new QuestionPresenceTracker();
    const sessionId = generateId() as UUID;
    const hookId = '00000000-0000-4000-8000-00000000aaaa' as UUID;
    const ptyId = '00000000-0000-4000-8000-00000000bbbb' as UUID;

    tracker.parkAwaitingPTY({
      id: hookId,
      sessionId: sessionId,
      text: 'hook text',
      options: [], // the shape that used to fall through to the PTY id
      timestamp: new Date().toISOString(),
    } as unknown as Question);

    const { merged } = tracker.consumeAndMerge({
      id: ptyId,
      sessionId: sessionId,
      text: 'hook text',
      options: [
    ...
    expect(merged.id).toBe(hookId);
    expect(merged.id).not.toBe(ptyId);
  });
});
