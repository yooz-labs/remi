...
describe('buildPrompt', () => {
  ...
  test('instructions injected as exception context, not a permission grant', () => {
    const [system] = buildPrompt('Bash', { command: 'bun test' }, 'Approve bun test runs.');
    expect(system?.content).toContain('USER GUIDANCE');
    expect(system?.content).toContain('EXCEPTION CONTEXT, NOT A PERMISSION GRANT');
    expect(system?.content).toContain('Approve bun test runs.');
    expect(system?.content).toContain('does not create a permission grant');
  });
  ...
  test('empty instructions omit the USER GUIDANCE section', () => {
    ...
    expect(system?.content).not.toContain('EXCEPTION CONTEXT, NOT A PERMISSION GRANT');
  });
  ...
  test('user guidance appears BEFORE the default guidelines as exception context', () => {
    // Guidance is contextual input for the model, not a permission grant. It is
    ...
    const userIdx = system?.content.indexOf('EXCEPTION CONTEXT, NOT A PERMISSION GRANT') ?? -1;
  ...
  test('ordering: USER GUIDANCE, then CONVERSATION CONTEXT block, then DEFAULT GUIDELINES', () => {
    ...
    const content = system?.content ?? '';
    const guidanceIdx = content.indexOf('EXCEPTION CONTEXT, NOT A PERMISSION GRANT');
  ...
  test('CONVERSATION CONTEXT block can appear without USER GUIDANCE', () => {
    ...
    const content = system?.content ?? '';
    expect(content).not.toContain('EXCEPTION CONTEXT, NOT A PERMISSION GRANT');
