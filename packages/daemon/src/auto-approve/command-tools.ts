 ...
 * the literal string, each independently:
 *
 ...
 * `"status"`) gets its text read as one. Every consequence of that runs in the
 * safe direction: a higher band and a broader catastrophic match both mean MORE
 * escalation, never a silent approval. Measured against this machine's
 * `~/.remi/hook-diag.jsonl` (111 hook events), no tool other than `Bash` carries
 * a `command` field at all, so the live cost today is zero.
 *
